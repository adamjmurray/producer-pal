...
export function useConversationHandlers(
  ...
  const handleDelete = useCallback(
    (id: string) => {
      ...
      // only ever target the live conversation (saveCurrentConversation reuses
      // the store's id, minting a new one only for a fork or brand-new chat,
      // which it then makes live), so a non-active row has no pending save to
      // resurrect. Stopping unconditionally would needlessly abort the user's
      // in-flight response and, mid-fork, let the teardown autosave overwrite
