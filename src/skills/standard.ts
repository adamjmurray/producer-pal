...
Some native devices expose class-level controls beyond their DeviceParameters. These flow through existing surfaces: **pseudo-params** (set via \`params\` {name, value} entries, read back in \`parameters\`), **\`actions\`** (function-call strings on update-device; discover a device's available actions via read-device \`include: ["actions"]\`), dynamic catalogs and (Wavetable) current mod routes via read-device \`include: ["options"]\`. Invalid enum values warn-and-skip and list the valid options.
...
- **Wavetable** \`filterRouting\` (serial/parallel/split), \`monoPoly\` (mono/poly), \`polyVoices\` (2/3/4/5/6/7/8/16), \`unisonMode\` (none/classic/shimmer/noise/phase-sync/position-spread/random-note), \`unisonVoiceCount\` (2-8), \`osc1Engine\`/\`osc2Engine\` (None/Fm/Classic/Modern), \`osc1Category\`/\`osc2Category\` + \`osc1Wavetable\`/\`osc2Wavetable\` (set category first; browse via options \`oscWavetableCategories\`, then \`osc1Wavetables\`/\`osc2Wavetables\` list the selected category's tables). Mod matrix via actions (see read-device \`include: ["actions"]\` for signatures); sources: Amp, Env 2, Env 3, LFO 1, LFO 2, Vel, Key, PB, Press, Mod, Rand, Note PB, Slide. read-device \`include: ["options"]\` returns current routes (\`modulations\`) and \`modulatableParameters\`.
- **Meld** \`monoPoly\` (mono/poly), \`polyVoices\` (1-6), \`unisonVoices\` (0-2).
- **Simpler** \`sample\` (file path), \`gainDb\` (sample level, 0 = unity), \`playbackMode\` (classic/one-shot/slicing), \`slicingPlaybackMode\` (mono/poly/thru), \`retrigger\`, \`voices\`; read-only \`multiSampleMode\`, \`estimatedPlaybackLength\`. Sample-editing actions (see read-device \`include: ["actions"]\`) operate on the active region — set the \`S Start\`/\`S Length\` DeviceParameters first to target a sub-range.
