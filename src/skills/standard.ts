...
- **Note-count operations** (change how many notes exist — write on their own line, NOT as a value: \`velocity = ratchet(2)\` errors): \`ratchet(N)\` divides each matched note into N equal pieces (a roll; \`ratchet(n/16)\` chops into 16th-note pieces instead), \`merge()\` spans all same-pitch matched notes into one sustained note. A selector scopes them (\`C1: ratchet(4)\`, \`2|*: merge()\`); a transform after a note op sees the rebuilt notes (so \`note.index\` re-derives). MIDI only
