...
- \`sync\` keyword (last arg on periodic waves) anchors phase to the arrangement timeline (continuous across clips) instead of clip start — arrangement clips only. On a session clip a synced wave can't resolve, so the whole transform is skipped (with a warning) and the note is left unchanged — it does NOT fall back to clip-relative. For a session-clip LFO, omit \`sync\`: phase is clip-relative by default
