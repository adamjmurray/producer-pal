...
**Simpler sample:** Load a sample with \`params: [{name: "sample", value: "<absolute file path>"}]\` on ppal-create-device or ppal-update-device; set its level with \`{name: "gainDb", value: <dB>}\` (0 = unity). Read-device exposes both as \`parameters\` entries: \`include: ["sample"]\` returns just the sample group (\`sample\`, \`gainDb\`); \`include: ["params"]\` returns the full set. Writes are skipped with a warning on non-Simpler devices and on Simpler in multi-sample mode.
