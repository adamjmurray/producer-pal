...
- **Wavetable** \`filterRouting\` (serial/parallel/split), \`monoPoly\` (mono/poly), \`polyVoices\` (2/3/4/5/6/7/8/16), \`unisonMode\` (none/classic/shimmer/noise/phase-sync/position-spread/random-note), \`unisonVoiceCount\` (2-8), \`osc1Engine\`/\`osc2Engine\` (None/Fm/Classic/Modern), \`osc1Category\`/\`osc2Category\` + \`osc1Wavetable\`/\`osc2Wavetable\` (browse via options \`osc1Wavetables\`/\`osc2Wavetables\`). Mod matrix via actions: \`setModulation('<targetParamName>', '<source>', <amount -1..1>)\`, \`clearModulation('<targetParamName>', '<source>')\`, \`addModulationTarget('<paramName>')\`. Sources: Amp, Env 2, Env 3, LFO 1, LFO 2, Vel, Key, PB, Press, Mod, Rand, Note PB, Slide. read-device \`include: ["options"]\` returns current routes (\`modulations\`) and \`modulatableParameters\`.
