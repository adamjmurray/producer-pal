...
- **Shorthand** (clears/simple sets): a single bar|beat-style token instead of \`param = value\` — \`v0\` delete · \`vN\`/\`v±N\`/\`vA-B\` velocity (range = humanized random, same as notes) · \`pN\`/\`p±N\` probability · \`n/4\` duration · \`C4\` remap pitch (one per line; a selector still applies, e.g. \`C1: v0\`). Preferred for clearing/deleting; use the full \`param op expr\` form for computed changes (\`+=\`, \`*=\`, waveforms, ramps)
