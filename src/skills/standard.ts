...
- **Value filter** \`where(...)\`: keep only notes whose properties satisfy a boolean test — \`where(note.velocity < 40): delete\` deletes quiet notes, \`where(note.velocity > 100): velocity += 20\` accents loud ones, \`where(note.probability < .5): delete\` thins. Build it from comparisons (\`> >= < <= == !=\`), booleans (\`&& || !\`), parens, arithmetic, and functions over note.velocity/deviation/duration/probability/pitch/start (\`note.duration\`/\`note.start\` in musical beats; RHS may be a number, note name, or \`n/8\`). Functions work too — \`where(abs(note.start - 4) < 1): velocity += 20\` (near beat 4, either side), \`where(min(note.velocity, note.deviation) > 80): ...\`. AND-combines with a pitch/time selector: \`C3-C5 where(note.velocity > 80): velocity += 20\`. Use \`<\`/\`>\` on the float props (duration, probability, start) and reserve \`==\`/\`!=\` for the integer ones (velocity, pitch). Note properties only (no note.index/count/next); all functions except legato/seq (they need the selection); not on note-count ops
