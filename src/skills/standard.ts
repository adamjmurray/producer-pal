...
**Dual meter per call:** in create/update-clip, \`arrangementStart\`/\`arrangementLength\` resolve against the **song** time signature, while \`start\`/\`firstStart\`/\`length\` resolve against the **clip** time signature. When a clip's meter differs from the song's, the same bar|beat literal denotes different absolute times across those params.
