...
- **Echo / copy notes to a later position** — \`repeat(offset, copies)\` echoes each matched note forward by \`offset\` (a note value like \`n/8\` or \`Nbar\`). \`copies\` defaults to 1, so \`repeat(n/8)\` adds one echo an 8th later and \`repeat(n/8, 3)\` adds three. It does NOT resize the clip — copies past the end stay hidden until you grow \`length\`; to double a loop AND lengthen the clip use update-clip's \`duplicateLoop\`
