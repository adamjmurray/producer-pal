...
**Note-count operations** — use these instead of rewriting the clip's notes by hand whenever the count changes in a regular way (roll, echo, cut, glue). MIDI only. Each is a whole statement on its own line, NOT a value (\`velocity = ratchet(2)\` errors). A selector scopes it (\`C1: ratchet(4)\`, \`2|*: merge()\`), and a transform on a later line sees the rebuilt notes (so \`note.index\` re-derives).
...
- **Echo / copy notes to a later position** — \`repeat(offset, copies)\` echoes each matched note forward by \`offset\` (a note value like \`n/8\` or \`<count>bar\`). \`copies\` defaults to 1, so \`repeat(n/8)\` adds one echo an 8th later and \`repeat(n/8, 3)\` adds three. It does NOT resize the clip — copies past the end stay hidden until you grow \`length\`; to double a loop AND lengthen the clip use update-clip's \`duplicateLoop\`
