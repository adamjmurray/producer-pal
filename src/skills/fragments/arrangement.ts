...
- Take-lane clips are append-only: on a clip already on a lane, \`update-clip\` (\`arrangementSplit\`, \`arrangementStart\`, \`arrangementLength\`, \`toPath\`) and \`ppal-delete\` warn+skip — Live's API can't move or remove a clip off a lane, so ask the user to do those in Live's UI. Moving a main-lane clip ONTO a lane works: \`update-clip\` with \`toPath: "t2/l+"\`.
