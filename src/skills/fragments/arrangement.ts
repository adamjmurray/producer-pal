...
- Variation workflow: one duplicate with explicit lanes, \`toPath: "t2/l0,t2/l1,t2/l2"\` (lanes are created up to the index, so start at l0) + \`transforms\` using \`clip.index\`/\`clipseq()\` to vary each copy. read-track \`arrangement-clips\` include lists \`takeLanes\` — each entry carries its \`path\` (e.g. \`t2/l0\`) and \`name\`.
