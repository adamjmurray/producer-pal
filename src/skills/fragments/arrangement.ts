...
duplicate's \`id\` takes a list, copying each source in turn: \`count\` applies to every one. Several sources take one destination each, in order, and nothing cycles — \`toPath: "t0[1|1],t0[17|1],t0[33|1]"\` gives each source its own bar. Only a bare position covers them all: \`id: "c1,c2,c3"\` with \`toPath: "[5|1]"\` (or \`arrangementStart: "5|1"\` alone) drops each clip at bar 5 on its own track. One \`t0[5|1]\`, one clip slot, or one position for several scenes is refused: the copies would bury each other. A copy landing on another source of the same call is refused too: copy that source first, in its own call. \`name\`, \`color\`, \`arrangementLength\` and \`clip.index\` count across every copy, not per source: one clip to \`[1|1],[3|1],[5|1]\` with \`arrangementLength: "2bar,2bar,4bar"\` lays it out A-A-AB.
