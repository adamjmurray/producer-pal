...
duplicate's \`id\` takes a list, copying each source in turn: \`count\` applies to every one. Several sources take one destination each, in order, and nothing cycles — \`toPath: "t0[1|1],t0[17|1],t0[33|1]"\` gives each source its own bar. Only a bare position covers them all: \`id: "c1,c2,c3"\` with \`toPath: "[5|1]"\` (or \`arrangementStart: "5|1"\` alone) drops each clip at bar 5 on its own track. One \`t0[5|1]\`, one clip slot, or one position for several scenes is refused: the copies would bury each other. A copy landing on a later source of the same call is refused too: list that source first, or copy it in its own call. \`name\`, \`color\`, \`arrangementLength\` and \`clip.index\` count across every copy, not per source: one clip to \`[1|1],[3|1],[5|1]\` with \`arrangementLength: "2bar,2bar,4bar"\` lays it out A-A-AB.
...
- A lane is a path segment: \`t2/l0\` is the track's first take lane. Arrangement only, and lanes up to the index are created as needed. Read \`takeLanes\` from read-track first so you know which lanes exist — none can be deleted. A stack of takes on one lane is \`toPath: "t2/l0[9|1],t2/l0[13|1]"\`.
...
- A whole lane's clips go onto another lane in one call: \`duplicate\` with \`type: "track"\` and a lane \`toPath\` (\`t2/l0\`, \`t2/l+\`) copies every clip at the position it already has. The source is a track (\`path: "t2"\`, its main lane) or a lane (\`path: "t2/l0"\`, or the lane's \`id\`), and a lane can't copy onto itself. A lane source with a bare track \`toPath\` (\`t2\`, its own track or another) promotes its clips onto that track's main lane instead, over the clips already there. Clips only — devices, routing, mixer settings and session clips stay behind — and the two tracks must match MIDI/audio type. \`name\`/\`color\` label the copied clips.
...
A lane is a path segment: \`t2/l0\` is the first lane, and lanes up to the index are created as needed. Arrangement only. Producer Pal can't delete or comp lanes — that's Live's UI, so read \`takeLanes\` first and use the next free index.`;
