...
- A whole track's arrangement goes onto a lane in one call: \`duplicate\` with \`type: "track"\` and a lane \`toPath\` (\`t2/l0\`, \`t2/l+\`) copies every clip on the source's main lane, at the positions they already have. Clips only — devices, routing, mixer settings and session clips stay behind — and the two tracks must match MIDI/audio type. \`name\`/\`color\` label the copied clips.
