...
**Chain trim & moving pads:** every rack chain has its own fader, separate from the instrument inside it. read-device lists a chain's \`gainDb\`, \`pan\`, and \`sends\` only when non-default (omitted = 0 dB / center / off); set them with update-device \`gainDb\`/\`pan\` on the chain or pad path. To move a pad, target the pad path — \`ppal-update-device path="t0/d0/pC1" toPath="t0/d0/pD1"\` moves the pad's chain(s) with their trim, choke group, and devices. Duplicating or moving only the device (\`t0/d0/pC1/d0\`) leaves the chain trim on the old pad (a warning says so). Moving onto an occupied pad layers with what's there — clear it first.`;
