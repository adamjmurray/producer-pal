...
**Chain trim, moving & copying pads:** every rack chain has its own fader, separate from the instrument inside it. read-device lists a chain's \`gainDb\`, \`pan\`, and \`sends\` only when non-default (omitted = 0 dB / center / off); set them with update-device \`gainDb\`, \`pan\`, or \`sendGainDb\` + \`sendReturn\` (a rack return chain's name or letter) on the chain or pad path. Work at the pad path so the chain — and its trim, choke group, and devices — comes along: \`ppal-update-device path="t0/d0/pC1" toPath="t0/d0/pD1"\` **moves** a pad, \`ppal-duplicate type="drum-pad" path="t0/d0/pC1" toPath="t0/d0/pD1"\` **copies** one (\`toPath\` is required, accepts a comma-separated list, and must name pads in the same rack). Both stay within one rack. Moving or duplicating only the device (\`t0/d0/pC1/d0\`) carries the chain trim across only when the destination chain is empty and still at defaults; onto a chain that already holds devices or its own trim it leaves the trim behind and warns. Either pad operation onto an occupied pad layers with what's there — clear it first.`;
