...
**Chain trim, moving & copying pads:** every rack chain has its own fader, separate from the instrument inside it. read-device lists a chain's \`gainDb\`, \`pan\`, and \`sends\` only when non-default (omitted = 0 dB / center / off); set them with update-device \`gainDb\`, \`pan\`, or \`sendGainDb\` + \`sendReturn\` (a rack return chain's \`id\`, name, or letter) on the chain or pad path. Work at the pad path so the chain — and its trim, choke group, and devices — comes along: \`ppal-update-device path="t0/d0/pC1" toPath="t0/d0/pD1"\` **moves** a pad, \`ppal-duplicate type="drum-pad" path="t0/d0/pC1" toPath="t0/d0/pD1"\` **copies** one (a pad \`id\` from read-device works as the source instead, but not alongside \`path\`; \`toPath\` is required, accepts a comma-separated list, and must name pads in the same rack). Both stay within one rack. Moving or duplicating only the device (\`t0/d0/pC1/d0\`) carries the chain trim across only within the same rack, and only when the destination chain is empty and still at defaults; anywhere else it leaves the trim behind and warns. Either pad operation onto an occupied pad layers with what's there — clear it first.

**Layered pads:** a bare pad path applies \`mute\`, \`solo\`, \`chokeGroup\`, \`mappedPitch\`, \`color\` and a move to every layer. \`name\`, \`gainDb\`, \`pan\` and sends belong to one layer, so on a pad holding more than one they are skipped with a warning listing the layer paths (\`t0/d0/pC1/c0\`, \`t0/d0/pC1/c1\`) — reissue on those. To remove one layer and keep the rest, \`ppal-delete type="chain" path="t0/d0/pC1/c1"\` (drum rack chains only — Live can't delete an Instrument Rack's).`;
