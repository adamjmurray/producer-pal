...
Make several with a comma-separated path list, one entry per object, in order: \`t+,t+,t+\` appends three tracks, \`s+,s+\` appends two scenes, \`t2,t2\` inserts two tracks at 2 with the second after the first. A value param whose description says "one per" (\`name\`, \`color\`, \`mute\`, \`gainDb\`, a new track's \`type\`, a scene's \`tempo\`, …) pairs with it 1:1; the rest apply to every object. With one target a comma is part of the value, not a separator: it's how a name like \`Verse, take 2\` is set.
