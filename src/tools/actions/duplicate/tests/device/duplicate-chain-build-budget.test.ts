...
describe("duplicate chain build budget", () => {
  // Pins today's cost, which is about 12 resolutions per device carried. Only
  // one of them has to scale: taking the temp track's first device, because
  // moving one out shifts the next into its place. The rest name things the
  // carry settles before it starts — the destination chain, that chain's mixer,
  // the track and the rack above it — and each is walked again per device.
  //
  // So these numbers are a baseline to ratchet down, not a target to defend.
  // Lowering one is the fix; raising one means a new repeat crept in.
