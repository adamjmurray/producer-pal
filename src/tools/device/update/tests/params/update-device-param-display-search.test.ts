...
describe("updateDevice - display-value search", () => {
  ...
  it("clamps to the raw max when the target is above the display range", () => {
    ...
    expect(capturedWarnings()).toContain(
      'updateDevice: id db-param param "Drive" only goes from -36.0 dB to 36.0 dB, so 99 was set to the nearest valid value.',
    );
  });
...
describe("updateDevice - a request Live silently drops", () => {
  ...
  it("warns when the value never changed", () => {
    ...
    expect(capturedWarnings()).toContain(
      'updateDevice: id word-param param "Vintage" was not changed — it still reads "Subtle". Live ignores a value outside the parameter\'s range.',
    );
  });
...
describe("updateDevice - a word at the max end of the range", () => {
  ...
  it("names the word when a target overshoots the numbers", () => {
    ...
    expect(capturedWarnings()).toContain(
      'updateDevice: id sentinel-param param "Release" only goes from 0.1 to 1.2 (or "A"), so 3 was set to the nearest valid value.',
    );
  });
});
