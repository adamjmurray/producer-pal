...
describe("updateDevice", () => {
  ...
  describe("params - sample (Simpler)", () => {
    it("should call load_file on sample for Simpler device", () => {
      liveApiGet.mockImplementation(function (prop) {
        // Simpler device (id 123)
        if (this._path === "id 123") {
          if (prop === "class_display_name") return ["Simpler"];
          if (prop === "sample") return ["id", "sample-obj"];
        }
        return [0];
      });
      ...
      const result = updateDevice({
        ids: "123",
        params: '{"sample": "/path/to/new-sample.wav"}',
      });

      expect(liveApiCall).toHaveBeenCalledWithThis(
        expect.objectContaining({ _path: "id sample-obj" }),
        "load_file",
        "/path/to/new-sample.wav",
      );
      expect(result).toEqual({ id: "123" });
    });

    it("should log error when sample key used on non-Simpler device", () => {
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});

      liveApiGet.mockImplementation(function (prop) {
        // Non-Simpler device (id 456)
        if (this._path === "id 456") {
      ...
      const result = updateDevice({
        ids: "456",
        params: '{"sample": "/path/to/sample.wav"}',
      });

      expect(consoleSpy).toHaveBeenCalledWith(
        "updateDevice: sample property only works on Simpler devices (got Operator)",
      );
      expect(result).toEqual({ id: "456" });
    ...
    it("should log error when Simpler has no sample object", () => {
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});

      liveApiGet.mockImplementation(function (prop) {
        // Simpler device (id 123) without sample children
        if (this._path === "id 123") {
          if (prop === "class_display_name") return ["Simpler"];
          if (prop === "sample") return []; // No sample children
        }
        return [0];
      });
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "updateDevice: could not access Simpler sample object",
      );
      expect(result).toEqual({ id: "123" });
