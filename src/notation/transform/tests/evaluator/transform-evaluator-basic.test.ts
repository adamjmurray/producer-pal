...
describe("Transform Evaluator", () => {
  ...
  describe("ramp waveform", () => {
    ...
    it("evaluates ramp at position 4 (end of clip, reaches end value)", () => {
      const result = evaluateTransform(
        "velocity += ramp(0, 1)",
        createContext({ position: 4, clipTimeRange: { start: 0, end: 4 } }),
      );
