...
describe("Modulation Evaluator", () => {
  describe("basic structure", () => {
    ...
    it("evaluates single parameter", () => {
      ...
      expect(result).toStrictEqual({ velocity: 10 });
    });
    ...
    it("evaluates multiple parameters", () => {
      ...
      expect(result).toStrictEqual({ velocity: 10, timing: 0.05 });
    });
  });
  ...
  describe("arithmetic operations", () => {
    ...
    it("respects operator precedence", () => {
      ...
      expect(result.velocity).toBe(20); // 10 + (5 * 2)
    });
  ...
  describe("cosine waveform", () => {
    it("evaluates cos at position 0 (starts at peak)", () => {
      ...
      expect(result.velocity).toBeCloseTo(1.0, 10);
    });
    ...
    it("evaluates cos at position 0.25 (quarter period)", () => {
      ...
      expect(result.velocity).toBeCloseTo(0.0, 10);
    });
    ...
    it("evaluates scaled cosine", () => {
      ...
      expect(result.velocity).toBeCloseTo(20.0, 10);
    });
  });
  ...
  describe("noise waveform", () => {
    ...
    it("evaluates scaled noise", () => {
      ...
      expect(result.velocity).toBeGreaterThanOrEqual(-10.0);
      expect(result.velocity).toBeLessThanOrEqual(10.0);
    });
  });
  ...
  describe("complex expressions", () => {
    it("evaluates unipolar envelope (20 + 20 * cos)", () => {
      ...
      expect(result.velocity).toBeCloseTo(40.0, 10); // 20 + 20 * 1.0
    });
    ...
    it("evaluates swing timing (0.05 * (cos(1t) - 1))", () => {
      ...
      expect(result.timing).toBeCloseTo(0.0, 10); // 0.05 * (1.0 - 1) = 0
    });
    ...
    it("evaluates multiple functions combined", () => {
      ...
      expect(result.velocity).toBeCloseTo(30.0, 10); // 20 * 1.0 + 10
    });

    it("evaluates amplitude modulation (cos * cos)", () => {
      const result = evaluateModulation("velocity: 30 * cos(4:0t) * cos(1t)", {
        position: 0,
        timeSig: { numerator: 4, denominator: 4 },
      });
      expect(result.velocity).toBeCloseTo(30.0, 10); // 30 * 1.0 * 1.0
    });
  });
...
probability: 0.2 * cos(0:2t)`;
      ...
      expect(result.velocity).toBeCloseTo(20.0, 10);
      expect(result.timing).toBeGreaterThanOrEqual(-0.05);
      expect(result.timing).toBeLessThanOrEqual(0.05);
...
timing: 0.05`;
      ...
      expect(result.timing).toBe(0.05);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
  ...
  describe("real-world examples from spec", () => {
    ...
    it("pulse width modulation: velocity: 20 * square(2t, 0, 0.25)", () => {
      const result = evaluateModulation("velocity: 20 * square(2t, 0, 0.25)", {
        position: 0,
        timeSig: { numerator: 4, denominator: 4 },
      });
      expect(result.velocity).toBe(20.0);
    });
    ...
    it("combined functions: velocity: 20 * cos(4:0t) + 10 * noise()", () => {
      ...
      expect(result.velocity).toBeGreaterThanOrEqual(10.0);
      expect(result.velocity).toBeLessThanOrEqual(30.0);
    });
  });
});
