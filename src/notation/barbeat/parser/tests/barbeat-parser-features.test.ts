...
describe("BarBeatScript Parser - parser features", () => {
  ...
  describe("advanced parser features", () => {
    ...
    it("handles zero values correctly (duration via 0/1 fraction)", () => {
      expect(parser.parse("1|1 v0 p0.0 n0/1 C0")).toStrictEqual([
        { bar: 1, beat: 1 },
        { velocity: 0 },
        { probability: 0.0 },
        { duration: 0.0 },
    ...
    it("handles maximum values correctly", () => {
      expect(parser.parse("999|999.999 v127 p1.0 n999/1 G8")).toStrictEqual([
  ...
  describe("integration", () => {
    it("handles real-world drum pattern example with probability and velocity range", () => {
      const input = `
        1|1 v100 n/16 p1.0 C1 v80-100 p0.8 Gb1
        1|1.5 p0.6 Gb1
        2|2 v90 p1.0 D1
        v100 p0.9 Gb1
      `;
