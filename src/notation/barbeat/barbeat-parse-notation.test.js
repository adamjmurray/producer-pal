...
describe("BarBeat parseNotation()", () => {
  ...
  it("handles time state changes", () => {
    ...
    expect(result).toEqual([
      ...
      { pitch: 64, start_time: 4, duration: 1, velocity: 100, probability: 1.0, velocity_deviation: 0 }, // bar 2, beat 1 (4 beats per bar)
    ]);
  });
  ...
  it("handles complex state combinations", () => {
    ...
    expect(result).toEqual([
      ...
      { pitch: 64, start_time: 1, duration: 1.0, velocity: 80, probability: 0.7, velocity_deviation: 40 },
      { pitch: 65, start_time: 1, duration: 1.0, velocity: 80, probability: 0.7, velocity_deviation: 40 },
    ]);
  });
  ...
  it("maintains state across multiple bar boundaries", () => {
    ...
    expect(result).toEqual([
      ...
      { pitch: 64, start_time: 16, duration: 0.5, velocity: 80, probability: 0.8, velocity_deviation: 0 }, // bar 5, beat 1 = (5-1)*4 + (1-1) = 16
    ]);
  });

  it("handles velocity range validation", () => {
    expect(() => parseNotation("v128-130 C3")).toThrow("Invalid velocity range 128-130");
    expect(() => parseNotation("v-1-100 C3")).toThrow();
  });

  it("handles probability range validation", () => {
    expect(() => parseNotation("p1.5 C3")).toThrow("Note probability 1.5 outside valid range 0.0-1.0");
  });
  ...
  it("handles enharmonic equivalents", () => {
    ...
    expect(result).toEqual([
      { pitch: 61, start_time: 0, duration: 1, velocity: 100, probability: 1.0, velocity_deviation: 0 }, // C#3
      { pitch: 61, start_time: 0, duration: 1, velocity: 100, probability: 1.0, velocity_deviation: 0 }, // Db3 (same as C#3)
      { pitch: 66, start_time: 0, duration: 1, velocity: 100, probability: 1.0, velocity_deviation: 0 }, // F#3
