...
export function addToolCreateClip(server, callLiveApi) {
  server.tool(
    ...
    "Creates MIDI clips in Session or Arranger view. " +
      ...
      "IMPORTANT: For Arrangement view clips, all timing parameters (startMarker, endMarker, loopStart, loopEnd) and note positions in the BarBeat notation are relative to the clip's start time, not the global arrangement timeline. A clip placed at arrangerStartTime '17|1' with notes starting at '1|1' will play those notes at global arrangement bar 17. " +
      "Clip length is set to the nearest whole beat after the last note end time. To ensure correct clip length, it is necessary to set endMarker and loopEnd (these should usually be the same).",
    {
      ...
      arrangerStartTime: z
        ...
        .optional()
        .describe(
          "Arranger view start time in bar|beat position format using pipe separator (e.g., '1|1' = first beat of first bar of the arrangement, '2|3.5' = halfway through third beat of second bar of the arrangement). Uses song's time signature for conversion. Required when view is 'Arranger'."
        ),
