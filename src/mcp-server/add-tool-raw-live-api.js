  ...
  server.tool(
      ...
      "DEVELOPMENT ONLY - not available in production builds.",
    {
      ...
      operations: z
        .array(
          z.object({
            type: z.enum(["get_property", "set_property", "call_method", "get", "set", "call", "goto", "info", "getProperty", "getChildIds", "exists", "getColor", "setColor"]).describe("Operation type: Core operations (get_property, set_property, call_method) or convenience shortcuts (get, set, call, goto, info, getProperty, getChildIds, exists, getColor, setColor)"),
            property: z.string().optional().describe("Property name for get_property/set_property/get/set/getProperty operations, or child type for getChildIds operations"),
            method: z.string().optional().describe("Method name for call_method/call operations"),
            args: z.array(z.any()).optional().describe("Arguments for call_method/call operations"),
            value: z.any().optional().describe("Value for set_property/set operations, path for goto operations, or color for setColor operations"),
          }),
        )
        .min(1)
        .max(50)
