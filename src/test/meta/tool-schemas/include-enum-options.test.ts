...
import {
  ALL_INCLUDE_OPTIONS,
  OPT_IN_ONLY_INCLUDE_OPTIONS,
} from "#src/tools/shared/tool-framework/include-params.ts";
import { toolDefReadTrack } from "#src/tools/track/read/read-track.def.ts";

// Three lists say what a read tool's `include` accepts: the enum in its
// `.def.ts` (what validates the call), ALL_INCLUDE_OPTIONS (what '*' expands
// to), and OPT_IN_ONLY_INCLUDE_OPTIONS (published, but never by '*'). When they
// drift, an option is reachable by wildcard and rejected by name — which is
...
describe("read tool include enums", () => {
  it.each(CASES)(
    ...
    (toolType, def) => {
      ...
      const optIn = OPT_IN_ONLY_INCLUDE_OPTIONS[toolType] ?? [];

      expect(options.length).toBeGreaterThan(0);
      expect(publishedEnumValues(def, "include").toSorted()).toStrictEqual(
        [...options, ...optIn, "*"].toSorted(),
      );
    },
  );

  it("leaves an opt-in-only option out of what '*' expands to", () => {
    for (const [toolType, optIn] of Object.entries(
      OPT_IN_ONLY_INCLUDE_OPTIONS,
    )) {
      expect(ALL_INCLUDE_OPTIONS[toolType]).not.toContain(optIn[0]);
    }
  });
});
