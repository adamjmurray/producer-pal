    ...
    test("reports a cache read on a later request", async ({ page }) => {
      ...
      await page.evaluate(
        ({ provider, apiKey, model }) => {
          ...
          // codeql[js/clear-text-storage-of-sensitive-data] -- test key mirrors the app's by-design localStorage provider config; not a production credential store
          localStorage.setItem(
            `producer_pal_provider_${provider}`,
            JSON.stringify({
              apiKey, // codeql[js/clear-text-storage-of-sensitive-data]
              model,
              thinking: "Default",
              temperature: 1.0,
              showThoughts: false,
