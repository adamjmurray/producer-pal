...
test.describe("OpenAI GPT-5.2", () => {
  test("Quick Connect with GPT-5.2", async ({ page }) => {
    const apiKey = process.env.OPENAI_KEY ?? process.env.OPENAI_API_KEY;

    if (!apiKey) {
      throw new Error(
        "OPENAI_KEY or OPENAI_API_KEY environment variable is required. Add it to your .env file.",
      );
    }
    ...
    // Select GPT-5.2 model (before API key to avoid state reset)
    const modelSelect = page.getByTestId("model-select");
    await modelSelect.selectOption("gpt-5.2-2025-12-11");
    ...
    console.log("Test passed: OpenAI GPT-5.2 Quick Connect successful");
