...
| `takeLane: "new"`                                    | gone — name the lane: `/l0`                     |
...
- `takeLane` counted from 1; the `l<n>` path segment counts from 0.
  ...
  all. `takeLane: "new"` is gone: name the lane by index (lanes are created up
  to it), reading the track's take lanes for the next free one.
