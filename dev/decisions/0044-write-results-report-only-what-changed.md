...
- **A different value → the value read back off the object**, published the way
  a read publishes it, plus a `note` naming the fields it applies to: "gainDb,
  ...
  and ladder notes it already had, which do say which.
...
- **A refusal about one target rides on that target's entry.** `leftPan`/
  ...
  rack macro owns are a `note` on the track now instead of a warning — with
...
- One comparison, one publisher and one note, used by the mixer, send, tempo and
  device-param write paths:
